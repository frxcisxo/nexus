...
import { TechnicalDebtMetrics, CodeSmell, FutureRisks } from '../types.js';
...
export class DebtAnalyzer {
   ...
   */
  async analyzeTechnicalDebt(): Promise<TechnicalDebtMetrics> {
   ...
   */
  async predictFutureRisks(debt: TechnicalDebtMetrics): Promise<FutureRisks> {
    const risks: FutureRisks = {
      likelyToBreak: [],
    ...
    if (debt.duplicationPercentage > 0.3) {
      risks.suggestions.push(
        `${(debt.duplicationPercentage * 100).toFixed(1)}% code duplication detected - extract common utilities`
      );
    }
    ...
    for (const [file, coverage] of Object.entries(debt.testCoverage)) {
      if (coverage < 0.5) {
        risks.suggestions.push(
          `${file} has only ${(coverage * 100).toFixed(0)}% test coverage - add more tests`
        );
      }
    }
    ...
    if (debt.securityVulnerabilities.length > 0) {
      risks.securityConcerns = debt.securityVulnerabilities.map(v =>
   ...
   */
  private async detectDuplication(files: string[]): Promise<number> {
   ...
   */
  private async analyzeTestCoverage(files: string[]): Promise<Record<string, number>> {
   ...
   */
  async findMostProblemFiles(metrics: TechnicalDebtMetrics): Promise<string[]> {
    const fileScores: Array<[string, number]> = [];
